// Spanish locale — /es — Token Calculator landing page
...
export const metadata = {
  title: "Calculadora de Tokens — Gratis para GPT, Claude y Gemini",
  description:
    "Calculadora de tokens gratis para todos los modelos de IA. Cuenta tokens, estima costos de API y visualiza la tokenización para GPT-4o, Claude, Gemini, DeepSeek y LLaMA — al instante en tu navegador.",
  keywords: [
    "calculadora de tokens",
    "LLM calculadora de costos",
    "GPT token calculadora",
    "IA token calculadora",
    "API costos calcular",
  ],
  alternates: {
    canonical: "/es",
...
export default function SpanishPage() {
  const softwareSchema = {
    ...
    name: "Calculadora de Tokens",
    description: "Calculadora de tokens gratis para todos los modelos de IA.",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Español", item: "/es" },
    ],
  };
  ...
  const models = MODELS.map((m) => ({
    ...
    color: m.color,
    contextFormatted: m.contextWindow >= 1_000_000
      ? `${(m.contextWindow / 1_000_000).toFixed(1)}M`
      : `${(m.contextWindow / 1_000).toFixed(1)}K`,
  }));
  ...
  return (
    <>
      ...
      <section className="hero container" aria-label="Introducción">
      ...
      <LocalizedCalculator labels={CALCULATOR_LABELS.es} />
        ...
        <div className="card">
          ...
          <div className="pricing-table-desktop" style={{ overflowX: "auto" }}>
            <table className="conversion-table" style={{ minWidth: '700px' }}>
              <thead>
                <tr>
                  <th>{t.pricing_table.model_col}</th>
                  <th>{t.pricing_table.provider_col}</th>
                  <th>{t.pricing_table.context_col}</th>
                  <th>{t.pricing_table.input_col}</th>
                  <th>{t.pricing_table.output_col}</th>
                </tr>
              </thead>
              <tbody>
              ...
              <div key={m.id} className="price-card-m">
                ...
                <div className="price-card-m__cols">
                  <div className="price-col">
                    <span className="price-col-label">Entrada</span>
                  ...
                  <div className="price-col">
                    <span className="price-col-label">Salida</span>
                    <span className="price-col-val">${m.outputPrice.toFixed(2)}</span>
                    <span className="price-col-unit">/1M tokens</span>
                  </div>
                  <div className="price-col">
                    <span className="price-col-label">Contexto</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '1rem', padding: '0.75rem 1rem', background: 'var(--bg-tertiary)', borderTop: '1px solid var(--border-subtle)', borderRadius: '8px', fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center' }}>
              <span style={{ fontWeight: 600, color: 'var(--text-primary)', marginRight: '0.25rem' }}>Proveedores:</span>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem', alignItems: 'center', borderTop: '1px dashed var(--border-subtle)', paddingTop: '0.5rem' }}>
              <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>Color del precio (Entrada):</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}><span style={{ color: 'var(--green)', fontWeight: 700 }}>$0.00</span> ≤ $0.50 (Económico)</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}><span style={{ color: 'var(--text-secondary)', fontWeight: 700 }}>$1.00</span> Gama media</div>
      ...
      {/* How It Works */}
      <section className="how-it-works" style={{ marginTop: '4rem', marginBottom: '4rem' }} aria-label="Cómo funciona">
        ...
        <div className="how-it-works__steps">
          <div className="step">
            ...
            <h3 className="step__title">{t.how_it_works.step1_title}</h3>
            <p className="step__desc">{t.how_it_works.step1_desc}</p>
          </div>
          <div className="step__connector">→</div>
          <div className="step">
            <div className="step__number">02</div>
            <div className="step__icon">🤖</div>
            <h3 className="step__title">{t.how_it_works.step2_title}</h3>
            <p className="step__desc">{t.how_it_works.step2_desc}</p>
          </div>
          <div className="step__connector">→</div>
          <div className="step">
            <div className="step__number">03</div>
            <div className="step__icon">💰</div>
            <h3 className="step__title">{t.how_it_works.step3_title}</h3>
            <p className="step__desc">{t.how_it_works.step3_desc}</p>
          </div>
        </div>
      </section>
      ...
      <section className="container" style={{ marginTop: "4rem", marginBottom: "4rem", padding: "0 1.5rem" }}>
        <article style={{ maxWidth: '100%', margin: "0 auto" }}>
          <h2 style={{ fontSize: "1.25rem", fontWeight: 700, marginBottom: "1rem" }}>
            {t.seo_content.title} <span style={{ color: 'var(--accent)' }}>{t.seo_content.title_accent}</span>
          </h2>
      ...
      <FAQ items={FAQ_DATA.es.items} labels={FAQ_DATA.es.labels} />
    </>
  );
}
