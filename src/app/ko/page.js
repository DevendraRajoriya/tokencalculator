...
export const metadata = {
  title: "토큰 계산기 — GPT, Claude, Gemini용 무료 LLM 토큰 카운터",
  description:
    "무료 토큰 계산기. GPT-4o, Claude, Gemini, DeepSeek, LLaMA의 토큰을 실시간으로 세고 API 비용을 추정합니다.",
  keywords: ["토큰 계산기", "LLM 토큰", "GPT 토큰 카운터", "AI 토큰 계산기"],
  alternates: {
    canonical: "/ko",
...
export default function KoreanPage() {
  const softwareSchema = {
    ...
    name: "토큰 계산기", applicationCategory: "DeveloperApplication", operatingSystem: "Web",
    offers: { "@type": "Offer", price: "0", priceCurrency: "USD" }, inLanguage: "ko",
  };
  ...
  return (
    <>
      ...
      <LocalizedCalculator labels={CALCULATOR_LABELS.ko} />
        ...
        <div className="card">
              ...
              <div key={m.id} className="price-card-m">
                ...
                <div className="price-card-m__cols">
                  <div className="price-col">
                    <span className="price-col-label">입력</span>
                  ...
                  <div className="price-col">
                    <span className="price-col-label">출력</span>
                    <span className="price-col-val">${m.outputPrice.toFixed(2)}</span>
                    <span className="price-col-unit">/1M tokens</span>
                  </div>
                  <div className="price-col">
                    <span className="price-col-label">컨텍스트</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '1rem', padding: '0.75rem 1rem', background: 'var(--bg-tertiary)', borderTop: '1px solid var(--border-subtle)', borderRadius: '8px', fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center' }}>
              <span style={{ fontWeight: 600, color: 'var(--text-primary)', marginRight: '0.25rem' }}>제공업체:</span>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem', alignItems: 'center', borderTop: '1px dashed var(--border-subtle)', paddingTop: '0.5rem' }}>
              <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>입력 가격 색상:</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}><span style={{ color: 'var(--green)', fontWeight: 700 }}>$0.00</span> ≤ $0.50 (저가형)</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}><span style={{ color: 'var(--text-secondary)', fontWeight: 700 }}>$1.00</span> 중간 가격대</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}><span style={{ color: 'var(--amber)', fontWeight: 700 }}>$5.00</span> ≥ $5.00 (프리미엄)</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <FAQ items={FAQ_DATA.ko.items} labels={FAQ_DATA.ko.labels} />
    </>
  );
}
