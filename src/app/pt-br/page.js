...
const t = ptBRMessages;

export const metadata = {
  title: "Calculadora de Tokens — Grátis para GPT, Claude e Gemini",
  description:
    "Calculadora de tokens grátis para todos os modelos de IA. Conte tokens, estime custos de API e visualize a tokenização para GPT-4o, Claude, Gemini — instantâneo no seu navegador.",
  keywords: ["calculadora de tokens", "LLM calculadora", "GPT token", "IA token calculadora"],
  alternates: {
    canonical: "/pt-br",
...
export default function PortuguesePage() {
  const softwareSchema = {
    ...
    name: "Calculadora de Tokens", applicationCategory: "DeveloperApplication", operatingSystem: "Web",
    offers: { "@type": "Offer", price: "0", priceCurrency: "USD" }, inLanguage: "pt-BR",
  };
  ...
  return (
    <>
      ...
      <LocalizedCalculator labels={CALCULATOR_LABELS["pt-BR"]} />
        ...
        <div className="card">
            ...
            {[...MODELS].sort((a, b) => a.inputPrice - b.inputPrice).map((m) => (
              <div key={m.id} className="price-card-m">
                ...
                <div className="price-card-m__cols">
                  ...
                  <div className="price-col">
                    <span className="price-col-label">Saída</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '1rem', padding: '0.75rem 1rem', background: 'var(--bg-tertiary)', borderTop: '1px solid var(--border-subtle)', borderRadius: '8px', fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center' }}>
              <span style={{ fontWeight: 600, color: 'var(--text-primary)', marginRight: '0.25rem' }}>Provedores:</span>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem', alignItems: 'center', borderTop: '1px dashed var(--border-subtle)', paddingTop: '0.5rem' }}>
              <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>Cor do preço (Entrada):</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}><span style={{ color: 'var(--green)', fontWeight: 700 }}>$0.00</span> ≤ $0.50 (Econômico)</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}><span style={{ color: 'var(--text-secondary)', fontWeight: 700 }}>$1.00</span> Intermediário</div>
      ...
      <FAQ items={FAQ_DATA["pt-BR"].items} labels={FAQ_DATA["pt-BR"].labels} />
    </>
  );
}
