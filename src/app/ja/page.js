// Japanese locale — /ja — Token Calculator landing page
...
export const metadata = {
  title: "トークン計算機 — GPT、Claude、Gemini向け無料LLMトークンカウンター",
  description:
    "無料のトークン計算機。GPT-4o、Claude、Gemini、DeepSeek、LLaMAのトークンをリアルタイムでカウントし、APIコストを見積もります。ブラウザで即座に実行。",
  keywords: ["トークン計算機", "LLMトークン", "GPTトークン", "AIトークンカウンター", "APIコスト計算"],
  alternates: {
    canonical: "/ja",
    languages: {
      "x-default": "/",
...
export default function JapanesePage() {
  const softwareSchema = {
    ...
    name: "トークン計算機",
    description: "無料のトークン計算機。すべてのAIモデルに対応。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "日本語", item: "/ja" },
    ],
  };
  ...
  return (
    <>
      ...
      <section className="hero container" aria-label="紹介">
      ...
      <LocalizedCalculator labels={CALCULATOR_LABELS.ja} />
        ...
        <div className="card">
              ...
              <div key={m.id} className="price-card-m">
                ...
                <div className="price-card-m__cols">
                  <div className="price-col">
                    <span className="price-col-label">入力</span>
                  ...
                  <div className="price-col">
                    <span className="price-col-label">出力</span>
                    <span className="price-col-val">${m.outputPrice.toFixed(2)}</span>
                    <span className="price-col-unit">/1M tokens</span>
                  </div>
                  <div className="price-col">
                    <span className="price-col-label">コンテキスト</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '1rem', padding: '0.75rem 1rem', background: 'var(--bg-tertiary)', borderTop: '1px solid var(--border-subtle)', borderRadius: '8px', fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center' }}>
              <span style={{ fontWeight: 600, color: 'var(--text-primary)', marginRight: '0.25rem' }}>プロバイダー:</span>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem', alignItems: 'center', borderTop: '1px dashed var(--border-subtle)', paddingTop: '0.5rem' }}>
              <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>入力価格の色:</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}><span style={{ color: 'var(--green)', fontWeight: 700 }}>$0.00</span> ≤ $0.50（格安）</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}><span style={{ color: 'var(--text-secondary)', fontWeight: 700 }}>$1.00</span> 中価格帯</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}><span style={{ color: 'var(--amber)', fontWeight: 700 }}>$5.00</span> ≥ $5.00（プレミアム）</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="container" style={{ marginTop: "4rem", marginBottom: "4rem", padding: "0 1.5rem" }}>
        <article style={{ maxWidth: '100%', margin: "0 auto" }}>
          ...
          <p style={{ fontSize: "0.9375rem", color: "var(--text-secondary)", lineHeight: 1.7, marginBottom: "1rem" }}>{t.seo_content.p2}</p>
          <p style={{ fontSize: "0.9375rem", color: "var(--text-secondary)", lineHeight: 1.7 }}>{t.seo_content.p3}</p>
        </article>
      </section>

      <FAQ items={FAQ_DATA.ja.items} labels={FAQ_DATA.ja.labels} />
    </>
  );
}
