...
export default function GeminiTabs({ exactNumbers, compactVisualizer, showAllModels, defaultText }) {
  const [activeTab, setActiveTab] = useState("gemini-3-1-pro");

  const providerModels = MODELS.filter(m => m.provider === 'Google');
  ...
  return (
    <div className="container" style={{ marginTop: "0.25rem", marginBottom: "0.5rem" }}>
      ...
      <ModelCalculator
        ...
        relatedModelIds={
          showAllModels ? MODELS.filter(m => m.provider === 'Google' && m.id !== activeTab).map(m => m.id) : []
        }
      />
    </div>
  );
