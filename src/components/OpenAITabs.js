...
export default function OpenAITabs({ exactNumbers, compactVisualizer, showAllModels, defaultText }) {
  const [activeTab, setActiveTab] = useState("gpt-5.4");

  const providerModels = MODELS.filter(m => m.provider === 'OpenAI');
  ...
  return (
    <div className="container" style={{ marginTop: "0.25rem", marginBottom: "0.5rem" }}>
      ...
      <ModelCalculator
        ...
        relatedModelIds={
          showAllModels ? MODELS.filter(m => m.provider === 'OpenAI' && m.id !== activeTab).map(m => m.id) : []
        }
      />
    </div>
  );
